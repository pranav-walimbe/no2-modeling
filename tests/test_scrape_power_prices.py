...
class PartitionWriteTest(unittest.TestCase):
    ...
    def test_checks_source_market_location_and_hour_coverage(self) -> None:
        window = month_windows(date(2024, 1, 1), date(2024, 1, 1))[0]
        rows = []
        for market in ("day_ahead", "real_time"):
            for interval_start in pd.date_range("2024-01-01", periods=24, freq="h", tz="UTC"):
                rows.append(
                    {
                        "iso": "TEST",
                        "market": market,
                        ...
                        "source": "gridstatus:test",
                    },
                )
        with tempfile.TemporaryDirectory() as directory:
            output_path = Path(directory) / "2024-01.parquet"
            pd.DataFrame(rows).to_parquet(output_path)

            self.assertTrue(_partition_covers_window(output_path, "TEST", window))
            incomplete = pd.DataFrame(rows).iloc[:-1]
            incomplete.to_parquet(output_path)
            self.assertFalse(_partition_covers_window(output_path, "TEST", window))
...
class ScrapeISOTest(unittest.TestCase):
    ...
    def test_writes_resumable_month_partition(self, create_source, _sleep) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = root / "TEST" / "2024-01.parquet"
            monthly = pd.read_parquet(output)
            self.assertEqual(len(monthly), 48)
            self.assertEqual(set(monthly["market"]), {"day_ahead", "real_time"})
            ...
            source.fetch.reset_mock()
            create_source.reset_mock()
            scrape_iso(
                iso="TEST",
                start=date(2024, 1, 1),
                end=date(2024, 1, 1),
