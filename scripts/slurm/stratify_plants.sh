...
aoi_scores="/global/home/users/pranavwalimbe/vis/stratification-aoi-scores-${SLURM_JOB_ID}.png"
...
srun python -u -m preprocessing.stratify_plants --aoi-score-output "${aoi_scores}" "$@"

if [[ ! -s "${aoi_scores}" ]]; then
    echo "Expected AOI score chart was not created: ${aoi_scores}" >&2
    exit 1
fi
...
printf '%s\n' \
    ...
    "AOI score chart: ${aoi_scores}" \
    | ssh -o BatchMode=yes -o ConnectTimeout=15 "${mail_host}" \
        "mailx -s 'NO2 stratification selected AOIs (${SLURM_JOB_ID})' -a '${aoi_scores}' '${recipient}'"
