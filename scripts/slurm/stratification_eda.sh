#!/bin/bash
#SBATCH --job-name=stratification-eda
...
# The summaries are single-threaded pandas passes, so the four cores buy the
# 32 GB needed to hold every split CSV and its grouped intermediates at once.
...
srun python -u -m eda.stratification_eda "$@"
