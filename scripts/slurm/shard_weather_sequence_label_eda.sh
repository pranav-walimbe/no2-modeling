#!/bin/bash
#SBATCH --job-name=weather-sequence-eda
...
output_dir="/global/home/users/pranavwalimbe/vis/weather-sequence-eda-${SLURM_JOB_ID}"
output_image="${output_dir}/weather_sequence_eda.png"
...
printf '%s\n' \
    ...
    'Each sample shows aligned NO2, standardized temperature, u-wind, and v-wind sequences plus its label.' \
    | ssh -o BatchMode=yes -o ConnectTimeout=15 "${mail_host}" \
        "mailx -s 'NO2 and weather sequence EDA (${SLURM_JOB_ID})' -a '${output_image}' '${recipient}'"
