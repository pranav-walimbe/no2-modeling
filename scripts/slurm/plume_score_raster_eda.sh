#!/bin/bash
#SBATCH --job-name=plume-score-eda
...
output_dir="/global/home/users/pranavwalimbe/vis/plume-score-raster-eda-${SLURM_JOB_ID}"
output_image="${output_dir}/plume_score_raster_eda.png"
...
srun python -u -m eda.plume_score_raster_eda "$@" --output "${output_image}"

test -s "${output_image}"
mail_log_offset=$(ssh -o BatchMode=yes "${mail_host}" stat -c %s "${mail_log}")
printf '%s\n' 'The requested plume-score-ordered NO2 raster EDA is attached.' \
    | ssh -o BatchMode=yes "${mail_host}" \
        "mailx -s 'Plume-score-ordered NO2 raster EDA (${SLURM_JOB_ID})' -a '${output_image}' '${recipient}'"
