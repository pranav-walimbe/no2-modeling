#!/bin/bash
#SBATCH --job-name=regrid-48-vs-24
...
output_dir="/global/home/users/pranavwalimbe/vis/regrid-resolution-comparison-${SLURM_JOB_ID}"
output_image="${output_dir}/regrid_48_vs_24.png"
output_manifest="${output_dir}/regrid_48_vs_24_manifest.csv"
...
srun python -u -m eda.regrid_resolution_comparison \
    --sample-count 20 \
...
printf '%s\n' \
    'Attached: 20 validation scans regridded directly at 48x48 and 24x24 over the same 72 km AOIs.' \
    'The samples are balanced by emissions-change class and use the same color scale within each pair.' \
    | ssh -o BatchMode=yes -o ConnectTimeout=15 "${mail_host}" \
        "mailx -s 'TEMPO regridding: 48x48 vs 24x24 (${SLURM_JOB_ID})' \
