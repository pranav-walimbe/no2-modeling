#!/bin/bash
#SBATCH --job-name=ema-timestep-eda
...
output_dir="/global/home/users/pranavwalimbe/vis/ema-timestep-eda-${SLURM_JOB_ID}"
output_image="${output_dir}/ema_timestep_comparison.png"
...
srun python -u -m eda.ema_timestep_eda \
    --sample-count 100000 \
    --output-dir "${output_dir}"

test -s "${output_image}"
test -s "${output_dir}/ema_timestep_sample.parquet"
test -s "${output_dir}/ema_timestep_summary.csv"
test -s "${output_dir}/metadata.json"

mail_log_offset=$(ssh -o BatchMode=yes -o ConnectTimeout=15 "${mail_host}" stat -c %s "${mail_log}")
printf '%s\n' \
    'Attached: EMA(current) - EMA(previous) distributions for T = 4, 5, and 6.' \
    'Every panel uses the same reproducible sample of 100,000 complete AOI-hour histories.' \
    'Each panel lists record counts and percentages above 25, 50, 75, and 100 lb in absolute EMA change.' \
    'The configured modeling default is T = 5.' \
    | ssh -o BatchMode=yes -o ConnectTimeout=15 "${mail_host}" \
        "mailx -s 'EMA timestep comparison (${SLURM_JOB_ID})' -a '${output_image}' '${recipient}'"
