#!/bin/bash
#SBATCH --job-name=flux-confidence-rasters
...
output_dir="/global/home/users/pranavwalimbe/vis/flux-confidence-rasters-${SLURM_JOB_ID}"
output_image="${output_dir}/flux_confidence_raster_samples.png"
...
srun python -u -m eda.flux_confidence_raster_samples "$@" --output "${output_image}"

test -s "${output_image}"
mail_log_offset=$(ssh -o BatchMode=yes -o ConnectTimeout=15 "${mail_host}" stat -c %s "${mail_log}")
printf '%s\n' 'The requested flux-confidence raster EDA is attached.' \
    | ssh -o BatchMode=yes -o ConnectTimeout=15 "${mail_host}" \
        "mailx -s 'Flux confidence and NO2 raster quality' -a '${output_image}' '${recipient}'"
