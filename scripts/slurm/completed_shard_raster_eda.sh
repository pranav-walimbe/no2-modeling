#!/bin/bash
#SBATCH --job-name=completed-shard-eda
...
output_dir="/global/home/users/pranavwalimbe/vis/completed-shard-raster-eda-${SLURM_JOB_ID}"
output_image="${output_dir}/completed_shard_raster_eda.png"
...
srun python -u -m eda.completed_shard_raster_eda "$@" --output "${output_image}"

test -s "${output_image}"
mail_log_offset=$(ssh -o BatchMode=yes "${mail_host}" stat -c %s "${mail_log}")
printf '%s\n' 'The requested unit-count-ordered completed-shard NO2 raster EDA is attached.' \
    | ssh -o BatchMode=yes "${mail_host}" \
        "mailx -s 'Unit-count-ordered shard NO2 EDA (${SLURM_JOB_ID})' -a '${output_image}' '${recipient}'"
