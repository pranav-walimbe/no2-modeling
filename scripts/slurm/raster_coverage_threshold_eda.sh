#!/bin/bash
#SBATCH --job-name=raster-coverage-eda
...
output_dir="/global/home/users/pranavwalimbe/vis/raster-coverage-threshold-eda-${SLURM_JOB_ID}"
report_path="${output_dir}/report.txt"
recipient="pranav.walimbe@berkeley.edu"

srun python -u -m eda.raster_coverage_threshold_eda \
    --sample-count 1000 \
...
test -s "${report_path}"
mailx -s "TEMPO raster coverage EDA ${SLURM_JOB_ID}" "${recipient}" < "${report_path}"
