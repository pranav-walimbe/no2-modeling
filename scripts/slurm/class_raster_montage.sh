#!/bin/bash
#SBATCH --job-name=class-raster-eda
...
output_dir="/global/home/users/pranavwalimbe/vis/class-raster-montage-${SLURM_JOB_ID}"
output_image="${output_dir}/class_raster_montage.png"
...
srun python -u -m eda.class_raster_montage \
    --split train \
...
test -s "${output_image}"
test -s "${output_dir}/class_raster_montage.csv"
mail_log_offset=$(ssh -o BatchMode=yes -o ConnectTimeout=15 "${mail_host}" stat -c %s "${mail_log}")
printf '%s\n' \
    'Attached: 40 current and delta NO2 raster pairs from the training dataset, with 20 samples from each emissions-change class.' \
    'Samples span five plume-score strata within each class and use shared channel scales.' \
    | ssh -o BatchMode=yes -o ConnectTimeout=15 "${mail_host}" \
        "mailx -s 'Balanced NO2 class raster montage (${SLURM_JOB_ID})' -a '${output_image}' '${recipient}'"
