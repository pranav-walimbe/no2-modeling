...
usage() {
    cat <<'EOF'
Usage: launch_dataset_generation.sh [--shard-size N] [--afterok JOB_ID] [-- CLI_ARGS ...]
...
echo "Launching dataset generation with shard size ${shard_size}"
launcher_args=(--shard-size "${shard_size}")
