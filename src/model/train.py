...
def move_batch_to_device(batch, device):
    """Move every tensor in a model batch to the selected device."""
...
def train_epoch(model, loader, optimizer, criterion, device):
    ...
    total_loss = 0.0
    for batch in loader:
        image, num_adj, prev_qtr_mass, u10, v10, label = move_batch_to_device(batch, device)
        optimizer.zero_grad()
...
def run_inference(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            image, num_adj, prev_qtr_mass, u10, v10, _ = move_batch_to_device(batch, device)
...
def main():
    ...
    train_eval_loader = DataLoader(train_dataset, shuffle=False, drop_last=False, **loader_options)
