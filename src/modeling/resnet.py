...
class MaskedNO2Stem(nn.Module):
    ...
    def forward(self, values: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            values: One normalized NO2 raster channel.
            mask: Binary validity mask aligned with the raster.

        Returns:
            Spatial features on propagated valid support.
        """
...
class NOxModel(nn.Module):
    ...
    def forward(self, image: torch.Tensor, tabular: torch.Tensor) -> torch.Tensor:
        """Predict the emissions-change logit.

        Args:
            image: Normalized raster and mask channels.
            tabular: Normalized scalar features.

        Returns:
            One binary-classification logit per record.
        """
        features = []
        if self.use_image:
