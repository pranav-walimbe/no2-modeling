...
def extract_tempo_patch(args):                                                                                                   
    ...
    try:                                    
        ...
        with nc.Dataset(fpath) as root:
            ...
            if frac_good < IMG_QA_FILTER or np.any(qa == 2):                                                                     
        ...
        # rotate patch so wind direction points upward in image                                                                  
        angle_deg = np.degrees(np.arctan2(row["era5_u10"], row["era5_v10"]))
        no2 = rotate(no2, -angle_deg, reshape=False, order=1)                                                                    
...
def process_split(df: pd.DataFrame, split: str, cities_gdf: gpd.GeoDataFrame):                                                   
    ...
    counts, bin_edges = np.histogram(labels, bins=100)
    bin_idx = np.clip(np.digitize(labels, bin_edges[:-1]) - 1, 0, 49)                                                            
    freq_weights = 1.0 / (counts[bin_idx] + 1e-10)
    freq_weights = freq_weights / freq_weights.mean()                                                                            
                                        
    # compute wind speed from u and v components                                                                                 
    wind_speed = np.sqrt(                                                                                                        
        df.loc[valid_idxs]["era5_u10"].values ** 2 + df.loc[valid_idxs]["era5_v10"].values ** 2                                                                               
    )                                   
                                                                                                                                
    # store dataset DataFrame with original columns, label, wind speed, and npy index                                            
    out_df = df.loc[valid_idxs].drop(columns=["lat_min", "lat_max", "lon_min", "lon_max", "era5_u10", "era5_v10"]).copy()
    out_df["npy_idx"] = range(n_store)                                                                                           
    out_df["split"] = split                                                                                                      
    out_df["plume_score"] = plume_scores
    out_df["wind_speed"] = wind_speed                                                                                          
...
def main():                                                                                                                      
    ...
    for split, df in splits.items():
        ...
        if not all(col in df.columns for col in ["era5_u10", "era5_v10"]):
